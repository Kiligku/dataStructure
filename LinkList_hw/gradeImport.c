...
StudentLinkedListNode *studentLinkedListCreate(char student_id[], int grade)
{
    ...
    strcpy_s(newnode->StuID, sizeof(newnode->StuID) / sizeof(newnode->StuID[0]), student_id);
    newnode->Grade = grade;
    return newnode;
}
...
StudentLinkedListNode *studentLinkedListAdd(StudentLinkedListNode *head, StudentLinkedListNode *node)
{
    ...
    {
        node->next = NULL;
        return node; // 如果链表为空，将该节点作为头结点
    }
    if (node == NULL)
    {
        printf("insertion failed!"); // 如果插入的结果为空指针，直接返回头结点
        return head;
    }
    node->next = head;
    return node;
...
StudentLinkedListNode *reverseLinkedList(StudentLinkedListNode *head)
{
    // TODO
    // 就地算法
    StudentLinkedListNode *p = head->next;
...
StudentLinkedListNode *findCrossBeginNode(StudentLinkedListNode *class1, StudentLinkedListNode *class2)
{
    ...
    StudentLinkedListNode *p = (len1 > len2) ? class1 : class2;
    StudentLinkedListNode *q = (len1 > len2) ? class2 : class1;
    StudentLinkedListNode *crossNode = NULL;
...
int main()
{
    ...
    while (~scanf("%d%d", &num1, &num2))
    {
        ...
        for (i = 0; i < num1; i++)
        {
            ...
            // printf("id: %s, grade: %d", student_id, grade);
